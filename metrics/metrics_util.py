...
def co_cp(partition_class_bcs_assignment, runtime_call_volume, result=None):
    """ Structural modularity quality """
    # higehr is better
...
def get_table_modularity(partition):
            ...
            for key1, values1 in micro_table.items():
                if key1 != key:
                    ...
                    for n in all_combinations:
                        if table_reltion_int[(n[0].lower(), n[1].lower())] == 1 or table_reltion_int[
                            (n[1].lower(), n[0].lower())] == 1:
                            graph_result.append((key, key1))
                            graph_result.append((key1, key))
